...
export default {
  ...
  methods:{
    ...
    markIt(word){
      if(this.item.mark[this.target].length === 4)return;
      let index = this.item.mark[this.target].indexOf(word);
          index === -1 ? this.item.mark[this.target].push(word) : this.item.mark[this.target].splice(index,1);
    },
    done(){
