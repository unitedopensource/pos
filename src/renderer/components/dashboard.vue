    ...
    methods: {
        ...
        go(grid) {
            ...
            switch (route) {
                ...
                case "setting":
                    ...
                    break;
                case "cashDrawer":
                    this.station.cashDrawer.enable ? this.cashDrawerAvailable() : this.cashDrawerUnavailable();
                    break;
                case "lock":
                    this.resetDashboard();
                    this.setOp(null);
        ...
        askSelfCashIn() {
            this.$dialog({ title: 'dialog.selfCashIn', msg: 'dialog.selfCashInTip' })
                .then(() => { this.countSelfCash() }).catch(() => { this.$q() })
        },
        countDrawerCash(amount) {
            if (isNumber(amount) && amount > 0) {
                ...
                }).then(amount => { this.countDrawerCash(amount) }).catch(() => { this.$q() })
            }
        },
        countSelfCash(amount) {
            if (isNumber(amount)) {
                ...
                new Promise((resolve, reject) => {
                    this.componentData = { resolve, reject };
                    this.component = "counter";
                }).then(amount => { this.countSelfCash(amount) }).catch(() => { this.$q() })
            }
        },
        acceptCashIn(amount) {
