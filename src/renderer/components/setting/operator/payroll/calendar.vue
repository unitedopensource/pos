            ...
            <div class="wrap">
                ...
                <div class="calendar">
                    <div class="week" v-for="(week,index) in calendar" :key="index">
                        <div class="day" v-for="(day,idx) in week.days" :key="idx" :class="{diff:isDiff(day)}" @click="init.resolve(day)">{{day | moment('D')}}</div>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        initialCalendar() {
            ...
            calendar.length === 5 && calendar.push({
                days: Array(7).fill(0).map((n, i) => date.clone().add(1, 'week').startOf('week').add(n + i, 'day'))
            });
        ...
        isDiff(day) {
            return moment(this.date).format("M") !== moment(day).format("M");
        }
    }
}
</script>
