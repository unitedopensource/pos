    ...
    <div class="window inputter">
      ...
      <div class="inner">
        <div class="display">
          <h5>{{$t('tip.tipFor')}}</h5>
          <span class="due">
            <span class="symbol">$</span>{{init.approve | decimal}}
...
export default {
  ...
  created() {
    if (this.init.tip) {
      this.tip = this.init.tip.toFixed(2);
      this.unit = "$";
  ...
  methods: {
    ...
    enter() {
      ...
      if (this.unit === "%") {
        tip = toFixed(this.init.approve * this.tip / 100, 2);
      }
    ...
    tooltip() {
      if (~~this.tip > 0) {
        if (this.unit === "%") {
          return this.$t(
            "text.calcTips",
            toFixed(this.init.approve * this.tip / 100, 2).toFixed(2)
          );
        } else {
