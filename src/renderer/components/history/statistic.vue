            ...
            <div class="inner">
                <table>
                    ...
                    <tr v-for="(ticket,index) in tickets" :key="index" :class="{voided:ticket.status === 0}">
                        ...
                        <td :title="$t('text.server')+': '+ticket.server">{{ticket.cashier}}</td>
...
export default {
  ...
  created() {
    let temp = this.init.hasOwnProperty("history")
      ? this.init.history
      : this.history;
    this.invoices = [];
    ...
    this.types = [
      ...new Set(this.invoices.map(invoice => invoice.type))
    ].reverse();
    this.settles = [
      ...new Set(this.invoices.map(invoice => invoice.payment.type))
    ].reverse();
    ...
    index === -1
      ...
      : this.settles.splice(index, 1, "all");
  },
  data() {
    return {
      types: [],
      ...
      this.tickets = this.invoices.filter(invoice => {
        ...
        let match2 =
          this.settleFilter === "all"
            ? true
            : invoice.payment.type === this.settleFilter;
      ...
      this.tickets = this.invoices.filter(invoice => {
        let match1 =
          this.settleFilter === "all"
            ...
            : invoice.payment.type === this.settleFilter;
        let match2 =
          this.typeFilter === "all" ? true : invoice.type === this.typeFilter;
