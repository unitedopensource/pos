<template>
    <main class="window">
        ...
        <nav>
            <div class="tab active" @click="tap('form',$event)">{{text('CUST_INFO')}}</div>
            <div class="tab" @click="tap('history',$event)">{{text('HISTORY')}}</div>
            <div class="tab" @click="tap('log',$event)">{{text('CALL_LOG')}}</div>
            <div class="tab" @click="tap('giftCard',$event)">{{text('GIFT_CARD')}}</div>
        </nav>
        ...
        tap(tab, e) {
            ...
            switch (tab) {
                ...
                case "history":
                    ...
                    break;
                case "log":
                    this.customer.phone.length === 10 && this.$router.push({ name: 'CallList' });
                    break;
                case "giftCard":
                    this.customer.phone.length === 10 && this.$router.push({ name: 'customerGiftCard' });
                    break;
            }
        },
        cancel() {
